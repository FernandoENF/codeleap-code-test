...
const PostCard: FC<PostCardProps> = ({
  ...
  const { setIsOpen: setDeleteOpen } = useContext(DeleteModalContext)
  const { setIsOpen: setEditOpen } = useContext(EditModalContext)

  const username = useSelector<IState, string>((state) => state.user.username)
  const date = dayjs(created_datetime).fromNow()
